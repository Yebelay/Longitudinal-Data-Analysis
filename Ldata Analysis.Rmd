...
##	Topics covered 
 ...
 2. ** <span style="color:purple">Day 2: Modelling of longitudinal data</span>**
...
### <span style="color:purple"> Advantages of modern longitudinal methods </span>
...
- Research question: <span style="color:red">How does treatment affect blood pressure?</span>
...
- To identify outliers-points outside the estimated probability contour
  at <span style="color:red">level 1-id/2</span> will be marked in the plot, we use the R function.
...
What’s the difference between ML and REML?
- ML estimates of variances are known to be biased in small samples
- the simplest case: Sample variance $Var(x) = \frac{\sum_{i=1}^{n-1} (x_i-\bar{x})^2}{n-1}$
to obtain an unbiased estimate we need to divide by $n−1$
because we estimate the mean
$\bar{x} = \frac{\sum_{i=1}^{n} x_i}{n}$

- The REML estimation is a generalization of this idea
- It provides unbiased estimates of the parameters in the
covariance matrix $V_i$ in small samples
...
- The default estimation method in mos'"t software (R, SAS)
...
2. Compound Symmetry Covariance Structure
...
4. Unstructured Covariance Structure
...
- The hypothesis is:
  ...
  where <span style="color:blue">$L$</span> is the contrasts matrix.

- The numerator df are always equal to the rank of the contrast matrix <span style="color:blue">$L$</span>.
...
- **Research question:** **style="color:red">Severity relative to treatment of TDO, coded as <span 0 (not severe) or 1 (severe)</span>**.
...
- Popular alternative approaches, like <span style="color:blue;">Generalized Estimating Equations (GEE)</span>, Alternating Logistic Regression (ALR), and Pseudo-Likelihood (PL) have been formulated.
---
...
- The parameters of <span style="color:blue">Generalized Linear models</span> are estimated using the maximum likelihood approach.
...
- In the previous session, we focused on <span style="color:blue">marginal models (GEE)</span>:
  
  - It can be seen as the extension of the marginal models for continuous data.

- In this session, we will see the <span style="color:blue">analogue of linear mixed models for categorical data</span>.
...
<span style="color:blue">**2.**</span> The conditional mean of $y_{iii}$ depends upon fixed and random effects via the following expression: 
$\log\left(\frac{\pi_{ij}}{1 - \pi_{ij}}\right) = x_{i}^T\beta + z_{i}^Tb_{i}$

<span style="color:blue">**3.**</span> The random effects follow a multivariate normal distribution with mean zero and variance-covariance matrix $D$.
...
- The three-part specification of GLMMs corresponds to a full specification of the distribution of the outcome $y_{ij}$.
  ...
  - This is in contrast to the GEE approach, which is a <span style="color:blue">semi-parametric method</span>.
...
- In linear mixed effects models, both terms in the integrand - $p(y_{i} / b_{i}; \theta)$ and $p(b_{i}; \theta)$ - are densities of (multivariate) normal distributions, and also because $y_{i}$ and $b_{i}$ are linearly related.

- The integral in the log-likelihood expression has a <span style="color:blue">closed-form solution</span>.
...
- From the two alternatives, methods that rely on <span style="color:blue">approximation of the integral (GQ, AGQ)</span> have been shown to be superior.
...
- Though they are (much) more computationally demanding, they have a parameter that controls the accuracy of the approximation:
  ...
  - In GQ rules, it is the number of quadrature points (nGQ=1) point is equivalent to the Laplace approximation.
 
  - AGQ needs fewer quadrature points than classical GQ but is more time-consuming.
 
  - The <span style="color:blue">Laplace approximation</span> is a good choice when dealing with many repeated measures per subject.

- The higher nGQ (nAGQ), the more accurate the approximation will be.
...
- For the Linear Mixed Model (LMM): $E(Y_{ij} / b_{i}) = X_{i}\beta + Z_{i}b_{i}$
  - The <span style="color:blue">marginal expectation</span> $E(Y_{ij}) = E \left(E(Y_{ij} / b_{i})\right) = X_{i}\beta$
  - $\beta$ in LMMs has both <span style="color:blue">conditional and marginal interpretation</span>.

- For the Generalized Linear Mixed Model (GLMM), this does not hold:
  - Expectation of the <span style="color:blue">conditional expectation</span> of the GLMM:
  
$$E(E(Y_{ij}/b_{i})) = E\left[\frac{\exp(X_{i}\beta + Z_{i}b_{i})}{1 + \exp(X_{i}\beta + Z_{i}b_{i})} \right] \neq \frac{\exp(X_{i}\beta + Z_{i}b_{i})}{1 + \exp(X_{i}\beta + Z_{i}b_{i})}$$
    
  - <span style="color:blue">Parameter vector $\beta$ now does not have a marginal interpretation</span>!
...
- Packages: <span style="color:blue">lme4</span> and <span style="color:blue">GLMMadaptive</span>

- The function that fits GLMMs in <span style="color:blue">lme4</span> is `glmer()` – this has similar syntax as the `lmer()` function that fits linear mixed models, namely:

  - `formula`: a <span style="color:blue">formula</span> specifying the <span style="color:blue">response vector</span>, the <span style="color:blue">fixed- and random-effects structure</span>

  - `data`: a <span style="color:blue">data frame</span> containing all the variables

  - `family`: a <span style="color:blue">family object</span> specifying the <span style="color:blue">distribution of the outcome</span> and the <span style="color:blue">link function</span>

  - `nAGQ`: the <span style="color:blue">number of quadrature points</span>
...
- We start with an elaborate specification of the <span style="color:blue">fixed-effects structure</span> that contains all the variables we wish to study, and potential <span style="color:blue">nonlinear</span> and <span style="color:blue">interaction terms</span>.

- Following that, we build up the <span style="color:blue">random-effects structure</span>, starting from <span style="color:blue">random intercepts</span>, and then potentially including <span style="color:blue">random slopes</span>, <span style="color:blue">quadratic slopes</span>, etc.

- At each step, we perform <span style="color:blue">Likelihood Ratio Tests (LRTs)</span> to determine if including the additional random effect improves the fit of the model.

- After choosing the <span style="color:blue">random-effects structure</span>, we return to the <span style="color:blue">fixed effects</span> and assess whether the specification can be <span style="color:blue">simplified</span>.

- Once again, we start by testing <span style="color:blue">complex terms</span> (i.e., interactions and nonlinear terms), and then proceed to <span style="color:blue">drop explanatory variables</span> if required.
...
- Hence, for <span style="color:blue">dichotomous data</span> and when we have few to moderate 
number of <span style="color:blue">repeated measurements</span> per subject, we often can only fit 
<span style="color:blue">random intercepts models</span>

---
**Summary: GEE**

<span style="color:blue">Coefficients</span> relating Y to X
Inference valid in large samples even if distribution of Y and/or <span style="color:blue">variance of Y</span> are incorrectly specified
Valid inference if data are <span style="color:blue">Missing Completely At Random (MCAR)</span> even if variance model is wrong

**GLMM**

<span style="color:blue">Coefficients</span> relating Y to X <span style="color:blue">conditional on b</span>
Valid inference generally requires correct specification of distribution of Y and of <span style="color:blue">variance of Y</span>
Valid inference if data are <span style="color:blue">Missing At Random (MAR)</span>
---
...
- <span style="color: blue;">`ar1`</span> for autoregressive order 1
- <span style="color: blue;">`unstructured`</span> for unstructured working correlation structure
...
A <span style="color:blue">random-effects model</span> for non-Gaussian longitudinal data is applied to the Jimma Infant Data. The following model is assumed for the mean structure:
...
**<span style="color:purple"> Advantages of MCAR:</span>**
...
**<span style="color:red"> disadvantage of MCAR:</span>**
...
- A concrete example of NMAR is when a student <span style="color:red">skips a tutorial lesson</span> because the student knows that the attendance would not be graded.

- In this situation, the probability of missingness (skipping the tutorial) depends on both the <span style="color:red">observed data</span> (whether the tutorial is graded or not) and the <span style="color:red">unobserved data</span> (whether the student actually attended the tutorial).
...
- Diagnosing <span style="color:red">NMAR</span> can be challenging since the reasons for missingness are <span style="color:red">unknown</span> and cannot be directly observed.

- Handling <span style="color:red">NMAR</span> requires <span style="color:red">careful consideration</span> and specialized methods to address missingness properly.

- Researchers often use <span style="color:red">sensitivity analyses</span> and model-based imputation techniques to account for the non-ignorable missing data mechanism in <span style="color:red">NMAR</span> scenarios.

- The goal is to explore how different assumptions about the missing data mechanism may impact the study's results and <span style="color:red">conclusions</span>.
...
# LOCF Imputation using dplyr and tidyr
missData <- missData %>%
  group_by(Subject) %>%
...
- The `method` argument in `mice()` allows you to specify the imputation method to be used. - Setting `method = "mean"` indicates that you want to perform mean imputation for the missing values.
...
```{r, warning=FALSE, message=FALSE, eval=FALSE}
# Extract imputed data in long format using tidyr
...
sleepstudy_miss$MissingReaction <- ifelse(is.na(sleepstudy_miss$Reaction), 1, 0)

# Fit a logistic regression model to predict the probability of missingness
logit_model <- glm(MissingReaction ~ Days + Subject, data = sleepstudy_miss, family = binomial())

# Calculate the inverse probability weights
sleepstudy_miss$Weights <- ifelse(is.na(sleepstudy_miss$Reaction), 1 / predict(logit_model, type = "response"), 1)

# Fit the GEE model using the weights
gee_model <- geeglm(Reaction ~ Days, data = sleepstudy_miss, id = Subject, corstr = "exchangeable", weights = Weights)

# Print the GEE model summary
summary(gee_model)
```

Now, let's use multiple imputation to handle missing data:

```{r}
# Multiple Imputation with mice package
# Here, we'll use predictive mean matching to impute the missing values

# Set the number of imputations
n_imputations <- 5

# Create the imputation model
imputation_model <- mice(sleepstudy_miss, method = "pmm", m = n_imputations)
...
# Initialize a list to store coefficient estimates from each imputed dataset
gee_coefficients <- list()

# Fit GEE on each imputed dataset and extract coefficients
for (i in 1:n_imputations) {
  imputed_dataset <- imputed_data[imputed_data$.imp == i, ]
  gee_model <- geeglm(Reaction ~ Days, data = imputed_dataset, id = Subject, corstr = "exchangeable")
  gee_coefficients[[i]] <- coef(gee_model)
}

# Combine the results from multiple imputed datasets by averaging the coefficients
combined_results <- exp(rowMeans(do.call(rbind, gee_coefficients)))
```
...
In this example, we compared the results of the weighted GEE approach with multiple imputation using predictive mean matching. Depending on the complexity of the dataset and the nature of missingness, you may need to adjust the methods or assumptions accordingly.

Keep in mind that handling missing data can be complex, and the specific approach you choose should be based on careful consideration of your data and research question. Additionally, the simulation of missing data in this example is for illustrative purposes; in real-world scenarios, you would work with actual missing data from your dataset.
